        require([
            ...
            function (
                ...
                var tablePolygons = document.getElementById('tableau');


                var intersectionGlobale = "@intersection".toLowerCase() == "true";
                var Lire = "@Lire".toLowerCase() == "true";
                var Cree = "@Cree".toLowerCase() == "true";
                var Supp = "@Supprimer".toLowerCase() == "true";
                var Modifier = "@Modifier".toLowerCase() == "true";
                ...
                var wkid3 = new SpatialReference(4326);
                ...
                fillSymbol.setColor(new Color([15, 250, 150, 0.35]));

                var symbolNewOne = new SimpleFillSymbol();
                symbolNewOne.setColor(new Color([150, 150, 150, 0.5]));
                ...
                var array = []; var PointsWKID1 = []; var ringsParcels = [];
                ...
                var json = [
                    [
                        ...
                        {
                            "x": 368652.42925060005,
                            "y": 376497.29569671635,
                            "spatialReference": {
                                "wkid": 26191
                            }
                        },
                ...
                CountDownIntervalle(2, 1);

                function CountDownIntervalle(secondes, ticksSecondes) {
                    var countDownDate = secondes * 1000;
                    var interval = ticksSecondes * 1000;
                    var aug = 0;

                    var x = setInterval(function () {


                        var distance = countDownDate - aug;
                        console.log(distance);
                        aug += interval;


                        if (distance <= ticksSecondes) {
                            clearInterval(x);
                            DbToMap();
                            //DO SOMETHING
                        }
                ...
                function DbToMap() {
                    ...
                    //            @foreach(var item in Model.DemDerg.parcell) {
                    //    @: ringsParcels.push([JSON.parse("@item.GeometryText")]);

                    //}



                    var polygonText = "@Model.DemDerg.COMMUNES_RSK.Shape.ToString()";
                    polygonText = polygonText.replace("POLYGON", "").replace("((", "").replace("))", "").split(", ");
                    ...
                    var polygonJson = {
                        "rings": [pol], "spatialReference": wkid3
                    };
                    var polygonCommune = new Polygon(polygonJson);
                    map.graphics.add(new Graphic(polygonCommune, symbolNewOne));

                    if (ringsParcels.length > 0) {

                        ringsParcels.forEach(myFunction);
                        function myFunction(item, index) {
                            DrawPolygon(item);
                        }
                        GetExtentsFromPolygons(ringsParcels);
                    }


                    //else {
                    //    GetExtentsFromPolygons([[pol]]);
                    //}
                }
                        ...
                        btn.onclick = function () {
                            ...
                            valider.onclick = function () {
                                ...
                                gsvc.project([pt2], wkid1, function (projectPoint) {
                                    ...
                                    PointsWKID1[id - 1] = p2;
                                    UpdateGraphicPoints(); CarteSetExtent();
                                });
                ...
                document.getElementById('alimBD').onclick = function () {
                    if (array.length < 4) {
                        ...
                        alert('indice obligatoires');
                    }
                    else if (numFon.value == "") {
                        alert('numero fonciere est obligatoirs');
                    }
                    else {
                        ...
                        var result = "["; var shapeText = "POLYGON ((";

                        array.forEach(myFunctionFille);
                        function myFunctionFille(itemfils, indexfils) {
                            var p = new Point(itemfils[0], itemfils[1], wkid1);

                            var ppqwe = webMercatorUtils.webMercatorToGeographic(p);


                            shapeText += ppqwe.x + " " + ppqwe.y + ",";

                            result += "[" + itemfils[0] + "," + itemfils[1] + "]";

                            if (indexfils != array.length - 1) { result += ","; }
                        }
                        var pqw = new Point(array[0][0], array[0][1], wkid1);

                        var ppqwee = webMercatorUtils.webMercatorToGeographic(pqw);

                        result += "]";

                        shapeText += + ppqwee.x + " " + ppqwee.y + "))";
                        ...
                        var _parcele_1 = {
                            ...
                            "ShapeText": shapeText,
                            "indice": indic.value,
                            //"Fk_DemDerog": @Model.DemDerg.Id_DemDerg,
                            "numFoncier": numFon.value
                        };
                        array = [];
                        $("#tbodyCoord tr").remove();

                        RecuperePolygons(ringsParcels);


                        var json__ = JSON.stringify(_parcele_1);
                        formData.append("Parcel", json__);

                        //formData.append("Id_DemDerg", @Model.DemDerg.Id_DemDerg);


                        $.ajax({
                            url: '@Url.Action("AddParcel","WorkflowDerogation")',
                            type: "POST",
                            contentType: false,
                            dataType: "json",
                            data: formData,
                            ...
                            success: function (data) {
                                var geomText = data.parcelle.geomText;
                                var intersects = data.parcelle.intersects;
                                var id = data.parcelle.id;
                                var ind = data.parcelle.indice;
                                var num_fonc = data.parcelle.numfoncier;
                                var global = data.global;
                                if (!global) {
                                    var tablePolygons = document.getElementById('tableau');
                                    var tablarowslength = tablePolygons.rows.length;
                                    var row = tablePolygons.insertRow();
                                    row.setAttribute("id", tablarowslength);
                                    var cell1 = row.insertCell(0);
                                    var cell2 = row.insertCell(1);
                                    var cell3 = row.insertCell(2);
                                    var cell4 = row.insertCell(3);
                                    var cell5 = row.insertCell(4);
                                    cell1.innerHTML = tablarowslength + 1;
                                    cell2.innerHTML = ind;
                                    cell3.innerHTML = num_fonc;
                                    if (intersects) {
                                        cell4.innerHTML = "<img src='https://localhost:44349/Content/documentation/img/valid.png' />";
                                    }
                                    else {
                                        cell4.innerHTML = "<img src='https://localhost:44349/Content/documentation/img/del.png' />";
                                    }
                                    if (Supp) {
                                        cell5.innerHTML = "<a class='btn btn-sm buttonsReapeateds' href='/WorkflowDerogation/DeleteParcelle?Id_parcelle=" +
                                            id + "&FK_DemDerg=" /*+@Model.DemDerg.Id_DemDerg*/ + "'><i class='fa fa-minus'></i></a>";
                                    }
                                    DrawPolygon([array]);
                                    actualisationTable(tablePolygons);
                                    GetExtentsFromPolygons(ringsParcels);
                                }
                                else {
                                    alert("parcelle deja existe pour ce demande de derogation!");
                                }
                            },
                            error: function () {
                                alert("ERREUR !!");
                ...
                document.getElementById('vider').onclick = function () {
                    ...
                    array = []; PointsWKID1 = [];
                    map.graphics.clear(); RecuperePolygons(ringsParcels);
                    $("#tbodyCoord tr").remove();
                }
                ...
                function UpdateGraphicPoints() {
                    map.graphics.clear();
                    RecuperePolygons(ringsParcels);
                    array.forEach(UpdatePoints);
                    function UpdatePoints(item, index) {
                ...
                function DrawPolygon(Ring) {
                    var polygonJson = {
                        "rings": Ring, "spatialReference": { "wkid": 102100, "latestWkid": 3857 }
                    };
                ...
                function RecuperePolygons(Rings) {
                    map.graphics.clear();
                    Rings.forEach(myFunction);
                ...
                function GetExtentsFromPolygons(Rings) {
                ...
                function passClicked(valeurAttr) {
                    ...
                    formData.append("enregistrer", valeurAttr);

                    $.ajax({
                        type: "Post",
                        url: '@Url.Action("SituationGeo", "WorkflowDerogation")',
                        contentType: false,
                        cache: false,
                        dataType: "json",
                        data: formData,
                        ...
                        success: function (data) {
                            var linkdata = data + "";
                            if (linkdata.includes("SituationGeo")) {
                                alert("les parcelles n'intersecte pas avec la commune rensignee!!")
                            }
                            else {
                ...
                function actualisationTable(table_Polygons) {
                    var lignes = table_Polygons.rows;
                    for (var i = 0; i < lignes.length; i++) {


                        var date = new Date(lignes[i].getAttribute('value'));
                        ...
                        lignes[i].setAttribute('temps', tempsResterSec);
                        lignes[i].setAttribute('backclr', backclr);
                        lignes[i].setAttribute('clr', clr);
                        lignes[i].style.backgroundColor = backclr;
                        lignes[i].style.color = clr;

                        lignes[i].onclick = function () {
                            for (var j = 0; j < lignes.length; j++) {
                                table_Polygons.rows[j].style.backgroundColor = table_Polygons.rows[j].getAttribute('backclr');
                                table_Polygons.rows[j].style.color = table_Polygons.rows[j].getAttribute('clr');
                                table_Polygons.rows[j].clicked = false;
                            ...
                            //ROW CLICKED
                            GetExtentsFromPolygons([ringsParcels[id]]);
