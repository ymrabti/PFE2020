require([
    ...
    function (
        ...
        arcgisUtils.createMap("464e3d0db8954e209dfe0dc6195330dc", "map").then(function (response) {
            ...
            var tableParcelles = document.getElementById('Parcelles');

            for (var i = 0; i < tableParcelles.rows.length; i++) {

                if (tableParcelles.rows[i].cells[4].innerHTML == 'Complet') {


                    fn1(tableParcelles.rows[i].id);
                    ZoomCommune = false;
                    break;
        ...
        document.getElementById('addCoord').onclick = function () {
            ...
            var wkid = new SpatialReference(3857);
        ...
        document.getElementById('Croisement').onclick = function () {
            ...
            function result(featureSet) {
                ...
                var Equipements = featureSet[4].features;
                ...
                if (Zonage.length != 0) {
                    ...
                    text_zonage = "";
                    text_zonage += "<b><p>Informations sur le zonage </p></b><table id='lots' class='table table-striped table-bordered'><tr style = 'background-color:#337ab7;color:white;' > <td><b>Zone</b></td> <td><b>Définition</b></td> <td><b>Pourcentage d occupation</b></td></tr > ";
                    ...
                    for (var i = 0; i < Zonage.length; i++) {
                        if (Zonage[i].attributes["zone"] != null || Zonage[i].attributes["definition"] != null) {
                            var ccc = geometryEngine.intersect(featureSet[1].features[i].geometry, union);

                            var pourcentage = (geometryEngine.geodesicArea(ccc, 'square-meters') * 100) / geometryEngine.geodesicArea(union, 'square-meters');
                            text_zonage += "<tr>";
                            //text_zonage += "<td>" + (j + 1) + "</td>"; j++;
                            text_zonage += "<td>" + Zonage[i].attributes["zone"] + "</td>";
                            text_zonage += "<td>" + Zonage[i].attributes["definition"] + "</td>";
                            text_zonage += "<td>" + pourcentage + "</td>";

                            //text_zonage += "<td><div id='Zonagemap'></div></td>";
                            text_zonage += "</tr>";
                        }
                    }
                    ...
                    arcgisUtils.createMap("464e3d0db8954e209dfe0dc6195330dc", "Zonagemap").then(function (response) {
                        mapp2 = response.map;



                        var extent = featureSet[1].features[0].geometry.getExtent().expand(0.1);
                        mapp2.setExtent(mapp1.extent);


                        var sfs = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,
                            new SimpleLineSymbol(SimpleLineSymbol.STYLE_DASHDOT,
                                new Color([255, 0, 0]), 2), new Color([130, 196, 144, 0.9]));
                        var sfs1 = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,
                            new SimpleLineSymbol(SimpleLineSymbol.STYLE_DASHDOT,
                                new Color([255, 0, 0]), 2), new Color([130, 173, 196, 0.9]));




                        mapp2.graphics.clear();
                        if (featureSet[1].features[0] != null) {
                            var graphic22 = new Graphic(geometryEngine.intersect(featureSet[1].features[0].geometry, union), sfs);

                            mapp2.graphics.add(graphic22);

                        }

                        if (featureSet[1].features[1] != null) {
                            var graphic223 = new Graphic(geometryEngine.intersect(featureSet[1].features[1].geometry, union), sfs1);

                            mapp2.graphics.add(graphic223);
                        }
                    });
                ...
                if (Secteur.length != 0) {
                    ...
                    text_zonage += "<b><p>Informations sur les Secteurs </p></b><table id='lotss' class='table table-striped table-bordered'><tr style = 'background-color:#337ab7;color:white;' ><td><b>Secteur</b></td><td><b>Définition</b></td> <td><b>Pourcentage d occupation</b></td></tr > ";
                    var j = 0;
                    for (var i = 0; i < Secteur.length; i++) {
                        if (Secteur[i].attributes["zone"] != null || Secteur[i].attributes["definition"] != null) {
                            var ccc = geometryEngine.intersect(Secteur[i].geometry, union);

                            var pourcentage = (geometryEngine.geodesicArea(ccc, 'square-meters') * 100) / geometryEngine.geodesicArea(union, 'square-meters');
                            text_zonage += "<tr>";
                            //text_zonage += "<td>" + (j + 1) + "</td>"; j++;
                            text_zonage += "<td>" + Secteur[i].attributes["zone_"] + "</td>";
                            text_zonage += "<td>" + Secteur[i].attributes["definition"] + "</td>";
                            text_zonage += "<td>" + pourcentage + "</td>";
                            ////text_zonage += "<td><div id='Secteurmap'></div></td>";
                            text_zonage += "</tr>";
                        }
                    }
                    text_zonage += "</table><br/>";
                    ...
                    var res = text_zonage.replace(/’/g, "'");
                    var dd = "<div><br/><div id='parag' >" + res + "</div></div>";
                    arcgisUtils.createMap("464e3d0db8954e209dfe0dc6195330dc", "Secteurmap").then(function (response) {
                        mapp3 = response.map;



                        var extent = featureSet[2].features[0].geometry.getExtent().expand(0.1);
                        mapp3.setExtent(mapp1.extent);


                        var sfs = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,
                            new SimpleLineSymbol(SimpleLineSymbol.STYLE_DASHDOT,
                                new Color([255, 0, 0]), 2), new Color([242, 15, 52, 0.9]));
                        var sfs1 = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,
                            new SimpleLineSymbol(SimpleLineSymbol.STYLE_DASHDOT,
                                new Color([255, 0, 0]), 2), new Color([242, 277, 15, 0.9]));




                        mapp3.graphics.clear();
                        if (featureSet[1].features[0] != null) {
                            var graphic22 = new Graphic(geometryEngine.intersect(featureSet[2].features[0].geometry, union), sfs);

                            mapp3.graphics.add(graphic22);

                        }

                        if (featureSet[1].features[1] != null) {
                            var graphic223 = new Graphic(geometryEngine.intersect(featureSet[2].features[1].geometry, union), sfs1);

                            mapp3.graphics.add(graphic223);
                        }
                    });
                    document.getElementById("lol").innerHTML = dd;
                ...
                if (Ilots.length != 0) {
                    ...
                    text_zonage += "<b><p>Informations sur les ilots </p></b><table id='lotsss' class='table table-striped table-bordered'><tr style = 'background-color:#337ab7;color:white;' ><td><b>secteur</b></td><td><b>arrondissement</b></td> <td><b>Pourcentage d occupation</b></td></tr > ";
                    var j = 0;
                    for (var i = 0; i < Ilots.length; i++) {
                        if (Ilots[i].attributes["secteur"] != null || Ilots[i].attributes["arrondissement"] != null) {
                            var ccc = geometryEngine.intersect(Ilots[i].geometry, union);
                ...
                if (Equipements.length != 0) {
                    ...
                    text_zonage += "<b><p>Informations sur les équipements </p></b><table id='lotsss' class='table table-striped table-bordered'><tr style = 'background-color:#337ab7;color:white;' ><td><b>dénomination</b></td><td><b>affectation</b></td> <td><b>Pourcentage d occupation</b></td></tr > ";
                    var j = 0;
                    for (var i = 0; i < Equipements.length; i++) {
                        if (Equipements[i].attributes["denomination"] != null || Equipements[i].attributes["affectation"] != null) {
                            var ccc = geometryEngine.intersect(Equipements[i].geometry, union);

                            var pourcentage = (geometryEngine.geodesicArea(ccc, 'square-meters') * 100) / geometryEngine.geodesicArea(union, 'square-meters');
                            text_zonage += "<tr>";
                            //text_zonage += "<td>" + (j + 1) + "</td>"; j++;
                            text_zonage += "<td>" + Equipements[i].attributes["denomination"] + "</td>";
                            text_zonage += "<td>" + Equipements[i].attributes["affectation"] + "</td>";
                            text_zonage += "<td>" + pourcentage + "</td>";
                            text_zonage += "</tr>";
                        }
                    }
